...
python -m pip install -r blackbox_finetune_recall50/requirements.txt

BASE_MODEL="${BASE_MODEL:-Qwen/Qwen2.5-0.5B-Instruct}"
CUDA_DEVICE="${CUDA_DEVICE:-0}"
export CUDA_VISIBLE_DEVICES="$CUDA_DEVICE"
export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"
python -m blackbox_finetune_recall50.gpu --cuda-device "$CUDA_DEVICE"
...
DEFAULT_DATA_DIR="blackbox_finetune_recall50/data_no_partial_week_$SAMPLE_MODE"
DEFAULT_VALIDATION_DATA_DIR="blackbox_finetune_recall50/data_evaluation_no_partial_week_$SAMPLE_MODE"
DATA_DIR="${DATA_DIR:-$DEFAULT_DATA_DIR}"
VALIDATION_DATA_DIR="${VALIDATION_DATA_DIR:-$DEFAULT_VALIDATION_DATA_DIR}"
if [[ "$SAMPLE_MODE" == "short" ]]; then
  DEFAULT_OUTPUT_DIR="blackbox_finetune_recall50/runs/qwen2.5-0.5b-blackbox-recall50-short-lora"
elif [[ "$SAMPLE_MODE" == "xlong" ]]; then
  DEFAULT_OUTPUT_DIR="blackbox_finetune_recall50/runs/qwen2.5-0.5b-blackbox-recall50-xlong-lora"
elif [[ "$SAMPLE_MODE" == "xxlong" ]]; then
  DEFAULT_OUTPUT_DIR="blackbox_finetune_recall50/runs/qwen2.5-0.5b-blackbox-recall50-xxlong-lora"
else
  DEFAULT_OUTPUT_DIR="blackbox_finetune_recall50/runs/qwen2.5-0.5b-blackbox-recall50-long-lora"
fi
