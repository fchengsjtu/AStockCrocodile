...
$VenvDir = if ($env:VENV_DIR) { $env:VENV_DIR } else { ".\.venv-blackbox-finetune-recall50" }
...
Invoke-Step -m pip install -r .\blackbox_finetune_recall50\requirements.txt
...
Invoke-Step -m blackbox_finetune_recall50.gpu --cuda-device $CudaDevice
Write-EnvironmentSnapshot
if ($Mode -eq "diagnose") {
  exit 0
}

$DataDir = if ($env:DATA_DIR) { $env:DATA_DIR } else { "blackbox_finetune_recall50/data_no_partial_week" }
$ValidationDir = if ($env:VALIDATION_DATA_DIR) { $env:VALIDATION_DATA_DIR } else { "blackbox_finetune_recall50/data_evaluation_no_partial_week" }
...
$DefaultOutputDir = if ($SampleMode -eq "short") { "blackbox_finetune_recall50/runs/qwen2.5-0.5b-blackbox-recall50-short-lora" } elseif ($SampleMode -eq "xlong") { "blackbox_finetune_recall50/runs/qwen2.5-0.5b-blackbox-recall50-xlong-lora" } elseif ($SampleMode -eq "xxlong") { "blackbox_finetune_recall50/runs/qwen2.5-0.5b-blackbox-recall50-xxlong-lora" } else { "blackbox_finetune_recall50/runs/qwen2.5-0.5b-blackbox-recall50-long-lora" }
...
$BuildArgs = @("-m", "blackbox_finetune_recall50.build_dataset", "--output-dir", $DataDir, "--start-date", $TrainStart, "--end-date", $TrainEnd, "--negative-ratio", $NegativeRatio, "--sample-mode", $SampleMode)
...
$ValArgs = @("-m", "blackbox_finetune_recall50.build_validation_dataset", "--output-dir", $ValidationDir, "--start-date", $ValidationStart, "--end-date", $ValidationEnd, "--negative-ratio", $NegativeRatio, "--sample-mode", $SampleMode)
...
$TrainArgs = @(
  '-m', 'blackbox_finetune_recall50.train', '--base-model', $BaseModel, '--data-dir', $DataDir, '--output-dir', $OutputDir,
...
Invoke-Step -m blackbox_finetune_recall50.evaluate --base-model $BaseModel --adapter-dir "$OutputDir/adapter" --data-dir $ValidationDir --threshold 0.50 --min-positive-recall $MinRecall --cuda-device $CudaDevice --max-seq-length $MaxSeqLength
Invoke-Step -m unittest tests.test_blackbox_finetune_recall50 -v
