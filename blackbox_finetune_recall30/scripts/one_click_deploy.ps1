...
Invoke-Step -m blackbox_finetune_recall30.train --base-model $BaseModel --data-dir $DataDir --output-dir $OutputDir --max-seq-length $MaxSeqLength --epochs $Epochs --batch-size 1 --gradient-accumulation-steps $GradSteps --learning-rate 2e-4 --train-seed $TrainSeed --cuda-device $CudaDevice --no-4bit
