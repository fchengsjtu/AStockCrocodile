...
$DataDir = if ($env:DATA_DIR) { $env:DATA_DIR } else { "blackbox_finetune_recall30/data_no_partial_week" }
$ValidationDir = if ($env:VALIDATION_DATA_DIR) { $env:VALIDATION_DATA_DIR } else { "blackbox_finetune_recall30/data_evaluation_no_partial_week" }
...
$DefaultOutputDir = if ($SampleMode -eq "short") { "blackbox_finetune_recall30/runs/qwen2.5-0.5b-blackbox-recall30-short-lora" } elseif ($SampleMode -eq "xlong") { "blackbox_finetune_recall30/runs/qwen2.5-0.5b-blackbox-recall30-xlong-lora" } elseif ($SampleMode -eq "xxlong") { "blackbox_finetune_recall30/runs/qwen2.5-0.5b-blackbox-recall30-xxlong-lora" } else { "blackbox_finetune_recall30/runs/qwen2.5-0.5b-blackbox-recall30-long-lora" }
...
$BuildArgs = @("-m", "blackbox_finetune_recall30.build_dataset", "--output-dir", $DataDir, "--start-date", $TrainStart, "--end-date", $TrainEnd, "--negative-ratio", $NegativeRatio, "--sample-mode", $SampleMode)
...
$ValArgs = @("-m", "blackbox_finetune_recall30.build_validation_dataset", "--output-dir", $ValidationDir, "--start-date", $ValidationStart, "--end-date", $ValidationEnd, "--negative-ratio", $NegativeRatio, "--sample-mode", $SampleMode)
