...
from blackbox_finetune_recall30.inference import load_model, score_prediction
...
def predict_day(
    ...
    with mysql_connect() as conn:
        ...
        print(f"blackbox recall30 predict date={anchor} symbols={len(symbols)} batches={len(batches)}", flush=True)
    ...
    if save_db:
        with mysql_connect() as conn:
            saved = save_top_predictions(conn, result, "blackbox_finetune_recall30", threshold, max_seq_length, save_top_n)
        print(json.dumps({"date": str(anchor), "saved_predictions": saved, "strategy": "blackbox_finetune_recall30"}, ensure_ascii=False), flush=True)
