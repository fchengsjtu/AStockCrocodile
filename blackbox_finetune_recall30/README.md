...
The one-click scripts reuse existing `train.jsonl` and `test.jsonl` files in `blackbox_finetune_recall30/data_partial_week` and `blackbox_finetune_recall30/data_validation_partial_week`. After the first full dataset build, later full runs skip the expensive sample materialization step and go straight to training/evaluation.
...
Training also caches tokenized samples under `blackbox_finetune_recall30/data_partial_week/tokenized`. If `train.jsonl`, `BASE_MODEL`, and `MAX_SEQ_LENGTH` are unchanged, later training runs load the tokenized cache and skip the slow tokenization pass.
...
Training automatically resumes from the latest `blackbox_finetune_recall30/runs/qwen2.5-0.5b-blackbox-recall30-lora/checkpoints/update-*` checkpoint. The log should show `resuming adapter from ...` and `start_update=N`. Disable automatic resume only when you intentionally want to restart from the base model:
...
$env:RESUME_ADAPTER_DIR='blackbox_finetune_recall30\runs\qwen2.5-0.5b-blackbox-recall30-lora\checkpoints\update-012000'
powershell -ExecutionPolicy Bypass -File .\blackbox_finetune_recall30\scripts\one_click_deploy.ps1 full
...
python -m blackbox_finetune_recall30.build_dataset `
  ...
  --end-date 20241231 `
  --negative-ratio 1.0 `
  --output-dir blackbox_finetune_recall30/data_partial_week `
  --daily-window 55 `
  --weekly-window 55 `
  --batch-size 80
```
...
python -m blackbox_finetune_recall30.build_validation_dataset `
  ...
  --end-date 20260430 `
  --negative-ratio 1.0 `
  --output-dir blackbox_finetune_recall30/data_validation_partial_week `
  --daily-window 55 `
  --weekly-window 55 `
  --batch-size 80
```
...
python -m blackbox_finetune_recall30.train \
  ...
  --data-dir blackbox_finetune_recall30/data_partial_week \
...
python -m blackbox_finetune_recall30.evaluate `
  ...
  --data-dir blackbox_finetune_recall30/data_validation_partial_week `
  --threshold 0.50 `
