...
$VenvDir = if ($env:VENV_DIR) { $env:VENV_DIR } else { ".\.venv-blackbox-finetune-down6_neutral" }
...
Invoke-Step -m pip install -r .\blackbox_finetune_down6_neutral\requirements.txt
...
Invoke-Step -m blackbox_finetune_down6_neutral.gpu --cuda-device $CudaDevice
Write-EnvironmentSnapshot
if ($Mode -eq "diagnose") {
  exit 0
}

$ModelTag = if ($env:MODEL_TAG) { $env:MODEL_TAG } else { "down6" }
...
$TrainSeed = if ($env:TRAIN_SEED) { $env:TRAIN_SEED } else { "937498347" }
...
$NeutralRatio = if ($env:NEUTRAL_RATIO) { $env:NEUTRAL_RATIO } elseif ($env:NEGATIVE_RATIO) { $env:NEGATIVE_RATIO } else { "9.0" }
$NeutralTagValue = $NeutralRatio.Replace(".", "_")
$NeutralTag = "neutral$NeutralTagValue"
$DefaultDataDir = "blackbox_finetune_down6_neutral/data_no_partial_week_${ModelTag}_${SampleMode}_${NeutralTag}"
$DefaultValidationDir = "blackbox_finetune_down6_neutral/data_evaluation_no_partial_week_${ModelTag}_${SampleMode}_${NeutralTag}"
...
$DefaultOutputDir = "blackbox_finetune_down6_neutral/runs/qwen2.5-0.5b-blackbox-${ModelTag}-$SampleMode-lora"
...
if ($Mode -eq "smoke") {
  ...
  $DefaultTrainStart = "20230101"
  $DefaultTrainEnd = "20241231"
  $DefaultValidationStart = "20260101"
  $DefaultValidationEnd = "20260529"
  $TrainStart = if ($env:TRAIN_START_DATE) { $env:TRAIN_START_DATE } else { $DefaultTrainStart }
  $TrainEnd = if ($env:TRAIN_END_DATE) { $env:TRAIN_END_DATE } else { $DefaultTrainEnd }
  $ValidationStart = if ($env:VALIDATION_START_DATE) { $env:VALIDATION_START_DATE } elseif ($env:TEST_START_DATE) { $env:TEST_START_DATE } else { $DefaultValidationStart }
  $ValidationEnd = if ($env:VALIDATION_END_DATE) { $env:VALIDATION_END_DATE } elseif ($env:TEST_END_DATE) { $env:TEST_END_DATE } else { $DefaultValidationEnd }
...
$BuildArgs = @("-m", "blackbox_finetune_down6_neutral.build_dataset", "--output-dir", $DataDir, "--start-date", $TrainStart, "--end-date", $TrainEnd, "--neutral-ratio", $NeutralRatio, "--sample-mode", $SampleMode)
...
$ValArgs = @("-m", "blackbox_finetune_down6_neutral.build_validation_dataset", "--output-dir", $ValidationDir, "--start-date", $ValidationStart, "--end-date", $ValidationEnd, "--neutral-ratio", $NeutralRatio, "--sample-mode", $SampleMode)
...
$TrainArgs = @(
  '-m', 'blackbox_finetune_down6_neutral.train', '--base-model', $BaseModel, '--data-dir', $DataDir, '--output-dir', $OutputDir,
