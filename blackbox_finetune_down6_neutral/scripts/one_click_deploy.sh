...
VENV_DIR="${VENV_DIR:-$HOME/.venvs/astock-blackbox-finetune-down6_neutral}"
...
python -m pip install -r blackbox_finetune_down6_neutral/requirements.txt
...
python -m blackbox_finetune_down6_neutral.gpu --cuda-device "$CUDA_DEVICE"
if [[ "$MODE" == "diagnose" ]]; then
  exit 0
fi
MODEL_TAG="${MODEL_TAG:-down6}"
...
NEUTRAL_RATIO="${NEUTRAL_RATIO:-${NEGATIVE_RATIO:-9.0}}"
NEUTRAL_TAG="$(python - "$NEUTRAL_RATIO" <<'PY'
import sys
...
DEFAULT_DATA_DIR="blackbox_finetune_down6_neutral/data_no_partial_week_${MODEL_TAG}_${SAMPLE_MODE}_${NEUTRAL_TAG}"
DEFAULT_VALIDATION_DATA_DIR="blackbox_finetune_down6_neutral/data_evaluation_no_partial_week_${MODEL_TAG}_${SAMPLE_MODE}_${NEUTRAL_TAG}"
DATA_DIR="${DATA_DIR:-$DEFAULT_DATA_DIR}"
VALIDATION_DATA_DIR="${VALIDATION_DATA_DIR:-$DEFAULT_VALIDATION_DATA_DIR}"
DEFAULT_OUTPUT_DIR="blackbox_finetune_down6_neutral/runs/qwen2.5-0.5b-blackbox-${MODEL_TAG}-${SAMPLE_MODE}-lora"
...
if [[ "$MODE" == "smoke" ]]; then
  ...
  DEFAULT_VALIDATION_START="20260101"
  DEFAULT_VALIDATION_END="20260529"
  TRAIN_START="${TRAIN_START_DATE:-$DEFAULT_TRAIN_START}"
  TRAIN_END="${TRAIN_END_DATE:-$DEFAULT_TRAIN_END}"
  VALIDATION_START="${VALIDATION_START_DATE:-${TEST_START_DATE:-$DEFAULT_VALIDATION_START}}"
  VALIDATION_END="${VALIDATION_END_DATE:-${TEST_END_DATE:-$DEFAULT_VALIDATION_END}}"
...
BUILD_ARGS=(--output-dir "$DATA_DIR" --start-date "$TRAIN_START" --end-date "$TRAIN_END" --neutral-ratio "$NEUTRAL_RATIO" --sample-mode "$SAMPLE_MODE")
VAL_ARGS=(--output-dir "$VALIDATION_DATA_DIR" --start-date "$VALIDATION_START" --end-date "$VALIDATION_END" --neutral-ratio "$NEUTRAL_RATIO" --sample-mode "$SAMPLE_MODE")
if [[ -n "$POS_LIMIT" ]]; then
...
TRAIN_DATASET_SIGNATURE="kind=down6_neutral_training;start=$TRAIN_START;end=$TRAIN_END;neutral_ratio=$NEUTRAL_RATIO;sample_mode=$SAMPLE_MODE;down6_limit=${POS_LIMIT:-all};seed=$TRAIN_SEED"
VALIDATION_DATASET_SIGNATURE="kind=down6_neutral_validation;start=$VALIDATION_START;end=$VALIDATION_END;neutral_ratio=$NEUTRAL_RATIO;sample_mode=$SAMPLE_MODE;down6_limit=${POS_LIMIT:-all};seed=$TRAIN_SEED"
DATASET_REBUILT=0
if [[ "$EXPLICIT_DATASETS_ENABLED" == "1" ]]; then
  ...
  run_dataset_build_if_needed "$DATA_DIR" training "${REBUILD_DATASET:-}" "$TRAIN_DATASET_SIGNATURE" python -m blackbox_finetune_down6_neutral.build_dataset "${BUILD_ARGS[@]}"
  run_dataset_build_if_needed "$VALIDATION_DATA_DIR" validation "${REBUILD_VALIDATION_DATASET:-${REBUILD_DATASET:-}}" "$VALIDATION_DATASET_SIGNATURE" python -m blackbox_finetune_down6_neutral.build_validation_dataset "${VAL_ARGS[@]}"
fi
...
TRAIN_HELP="$(python -m blackbox_finetune_down6_neutral.train --help 2>&1 || true)"
...
python -m unittest tests.test_blackbox_finetune_down6_neutral -v
