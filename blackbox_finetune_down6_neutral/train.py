...
from blackbox_finetune_down6_neutral.inference import score_prediction
...
def _missing_dataset_error(data_dir: Path) -> FileNotFoundError:
    return FileNotFoundError(f"missing dataset files under {data_dir}; run blackbox_finetune_down6_neutral.build_dataset first")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fine-tune Qwen2.5 for down6_neutral black-box stock classification")
    ...
    parser.add_argument("--no-4bit", action="store_true", help="Accepted for script compatibility; down6_neutral Windows training uses fp16 LoRA on RTX3060.")
