# Black-Box Qwen Fine-Tuning, Recall 40
...
- Required target: `positive_recall >= 40%`.
...
powershell -ExecutionPolicy Bypass -File .\blackbox_finetune_recall40\scripts\one_click_deploy.ps1 smoke
```
...
```powershell
cd D:\Documents\StockInfoCrawler
powershell -ExecutionPolicy Bypass -File .\blackbox_finetune_recall40\scripts\one_click_deploy.ps1 diagnose
```
...
```powershell
cd D:\Documents\StockInfoCrawler
powershell -ExecutionPolicy Bypass -File .\blackbox_finetune_recall40\scripts\one_click_deploy.ps1 full
```

The one-click scripts reuse existing `train.jsonl` and `test.jsonl` files in `blackbox_finetune_recall40/data` and `blackbox_finetune_recall40/data_validation`. After the first full dataset build, later full runs skip the expensive sample materialization step and go straight to training/evaluation.
...
Training also caches tokenized samples under `blackbox_finetune_recall40/data/tokenized`. If `train.jsonl`, `BASE_MODEL`, and `MAX_SEQ_LENGTH` are unchanged, later training runs load the tokenized cache and skip the slow tokenization pass.
...
Training automatically resumes from the latest `blackbox_finetune_recall40/runs/qwen2.5-0.5b-blackbox-recall40-lora/checkpoints/update-*` checkpoint. The log should show `resuming adapter from ...` and `start_update=N`. Disable automatic resume only when you intentionally want to restart from the base model:
...
$env:RESUME_ADAPTER_DIR='blackbox_finetune_recall40\runs\qwen2.5-0.5b-blackbox-recall40-lora\checkpoints\update-012000'
powershell -ExecutionPolicy Bypass -File .\blackbox_finetune_recall40\scripts\one_click_deploy.ps1 full
...
bash blackbox_finetune_recall40/scripts/one_click_deploy.sh full
```
...
```powershell
python -m blackbox_finetune_recall40.build_dataset `
  --start-date 20110101 `
  --end-date 20251231 `
  --negative-ratio 1.0 `
  --output-dir blackbox_finetune_recall40/data `
  --daily-window 55 `
  --weekly-window 55 `
  --batch-size 80
```
...
```powershell
python -m blackbox_finetune_recall40.build_validation_dataset `
  --start-date 20260101 `
  --end-date 20260430 `
  --negative-ratio 1.0 `
  --output-dir blackbox_finetune_recall40/data_validation `
  --daily-window 55 `
  --weekly-window 55 `
  --batch-size 80
```
...
```bash
python -m blackbox_finetune_recall40.train \
  --base-model Qwen/Qwen2.5-0.5B-Instruct \
  --data-dir blackbox_finetune_recall40/data \
  --output-dir blackbox_finetune_recall40/runs/qwen2.5-0.5b-blackbox-recall40-lora \
  --max-seq-length 2048 \
  --epochs 1 \
  --batch-size 1 \
...
Evaluate and enforce the `40%` positive-recall target:

```powershell
python -m blackbox_finetune_recall40.evaluate `
  --base-model Qwen/Qwen2.5-0.5B-Instruct `
  --adapter-dir blackbox_finetune_recall40/runs/qwen2.5-0.5b-blackbox-recall40-lora/adapter `
  --data-dir blackbox_finetune_recall40/data_validation `
  --threshold 0.50 `
  --min-positive-recall 0.40 `
  --max-seq-length 512 `
  --cuda-device 0
```
...
```powershell
python -m blackbox_finetune_recall40.predict_day `
  --date 20260514 `
  --adapter-dir blackbox_finetune_recall40/runs/qwen2.5-0.5b-blackbox-recall40-lora/adapter `
  --threshold 0.50 `
  --max-seq-length 512 `
  --cuda-device 0 `
  --limit 20 `
  --output data\blackbox_recall40_predictions_20260514.csv
```
