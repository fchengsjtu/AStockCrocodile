...
from blackbox_finetune_recall40.common import (
    DEFAULT_BASE_MODEL,
    DEFAULT_OUTPUT_DIR,
    DEFAULT_SAMPLE_MODE,
    DEFAULT_WINDOW,
...
from blackbox_finetune_recall40.gpu import prepare_rtx3060
from blackbox_finetune_recall40.inference import load_model, score_prediction
...
def predict_day(
    ...
    with mysql_connect() as conn:
        ...
        print(f"blackbox recall40 predict date={anchor} symbols={len(symbols)} batches={len(batches)}", flush=True)
    ...
    if save_db:
        with mysql_connect() as conn:
            saved = save_top_predictions(conn, result, "blackbox_finetune_recall40", threshold, max_seq_length, save_top_n)
        print(json.dumps({"date": str(anchor), "saved_predictions": saved, "strategy": "blackbox_finetune_recall40"}, ensure_ascii=False), flush=True)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Predict one trading day with recall40 black-box model")
