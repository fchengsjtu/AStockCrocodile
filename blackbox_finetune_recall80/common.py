...
DEFAULT_OUTPUT_DIR_XLONG = Path("blackbox_finetune_recall80") / "runs" / "qwen2.5-0.5b-blackbox-recall80-xlong-lora"
DEFAULT_OUTPUT_DIR_XXLONG = Path("blackbox_finetune_recall80") / "runs" / "qwen2.5-0.5b-blackbox-recall80-xxlong-lora"
...
def default_data_dir(sample_mode: str | None = None) -> Path:
    ...
    return Path("blackbox_finetune_recall80") / f"data_no_partial_week_{mode}"


def default_validation_dir(sample_mode: str | None = None) -> Path:
    mode = normalize_sample_mode(sample_mode or os.environ.get("SAMPLE_MODE"))
    return Path("blackbox_finetune_recall80") / f"data_evaluation_no_partial_week_{mode}"
