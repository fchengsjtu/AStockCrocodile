...
VENV_DIR="${VENV_DIR:-$HOME/.venvs/astock-blackbox-finetune-recall80}"
...
python -m pip install -r blackbox_finetune_recall80/requirements.txt

BASE_MODEL="${BASE_MODEL:-Qwen/Qwen2.5-0.5B-Instruct}"
CUDA_DEVICE="${CUDA_DEVICE:-0}"
export CUDA_VISIBLE_DEVICES="$CUDA_DEVICE"
export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"
python -m blackbox_finetune_recall80.gpu --cuda-device "$CUDA_DEVICE"
...
DEFAULT_DATA_DIR="blackbox_finetune_recall80/data_no_partial_week_$SAMPLE_MODE"
DEFAULT_VALIDATION_DATA_DIR="blackbox_finetune_recall80/data_evaluation_no_partial_week_$SAMPLE_MODE"
DATA_DIR="${DATA_DIR:-$DEFAULT_DATA_DIR}"
VALIDATION_DATA_DIR="${VALIDATION_DATA_DIR:-$DEFAULT_VALIDATION_DATA_DIR}"
if [[ "$SAMPLE_MODE" == "short" ]]; then
  DEFAULT_OUTPUT_DIR="blackbox_finetune_recall80/runs/qwen2.5-0.5b-blackbox-recall80-short-lora"
elif [[ "$SAMPLE_MODE" == "xlong" ]]; then
  DEFAULT_OUTPUT_DIR="blackbox_finetune_recall80/runs/qwen2.5-0.5b-blackbox-recall80-xlong-lora"
elif [[ "$SAMPLE_MODE" == "xxlong" ]]; then
  DEFAULT_OUTPUT_DIR="blackbox_finetune_recall80/runs/qwen2.5-0.5b-blackbox-recall80-xxlong-lora"
else
  DEFAULT_OUTPUT_DIR="blackbox_finetune_recall80/runs/qwen2.5-0.5b-blackbox-recall80-long-lora"
fi
...
if [[ "$MODE" == "smoke" ]]; then
  DEFAULT_TRAIN_START="20110101"
  DEFAULT_TRAIN_END="20151231"
...
run_dataset_build_if_needed "$DATA_DIR" training "${REBUILD_DATASET:-}" python -m blackbox_finetune_recall80.build_dataset "${BUILD_ARGS[@]}"
run_dataset_build_if_needed "$VALIDATION_DATA_DIR" validation "${REBUILD_VALIDATION_DATASET:-${REBUILD_DATASET:-}}" python -m blackbox_finetune_recall80.build_validation_dataset "${VAL_ARGS[@]}"
...
python -m blackbox_finetune_recall80.train "${TRAIN_ARGS[@]}"
python -m blackbox_finetune_recall80.evaluate --base-model "$BASE_MODEL" --adapter-dir "$OUTPUT_DIR/adapter" --data-dir "$VALIDATION_DATA_DIR" --threshold 0.50 --min-positive-recall "$MIN_POSITIVE_RECALL" --cuda-device "$CUDA_DEVICE" --max-seq-length "$MAX_SEQ_LENGTH"
python -m unittest tests.test_blackbox_finetune_recall80 -v
