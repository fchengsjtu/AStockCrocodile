...
$VenvDir = if ($env:VENV_DIR) { $env:VENV_DIR } else { ".\.venv-blackbox-finetune-recall80" }
...
Invoke-Step -m pip install -r .\blackbox_finetune_recall80\requirements.txt
...
Invoke-Step -m blackbox_finetune_recall80.gpu --cuda-device $CudaDevice
...
$MinSeqLengthOnOom = if ($env:MIN_SEQ_LENGTH_ON_OOM) { $env:MIN_SEQ_LENGTH_ON_OOM } else { "512" }
$OomShrinkFactor = if ($env:OOM_SHRINK_FACTOR) { $env:OOM_SHRINK_FACTOR } else { "0.5" }
...
$DefaultDataDir = "blackbox_finetune_recall80/data_no_partial_week_$SampleMode"
$DefaultValidationDir = "blackbox_finetune_recall80/data_evaluation_no_partial_week_$SampleMode"
...
$DefaultOutputDir = if ($SampleMode -eq "short") { "blackbox_finetune_recall80/runs/qwen2.5-0.5b-blackbox-recall80-short-lora" } elseif ($SampleMode -eq "xlong") { "blackbox_finetune_recall80/runs/qwen2.5-0.5b-blackbox-recall80-xlong-lora" } elseif ($SampleMode -eq "xxlong") { "blackbox_finetune_recall80/runs/qwen2.5-0.5b-blackbox-recall80-xxlong-lora" } else { "blackbox_finetune_recall80/runs/qwen2.5-0.5b-blackbox-recall80-long-lora" }
...
$BuildArgs = @("-m", "blackbox_finetune_recall80.build_dataset", "--output-dir", $DataDir, "--start-date", $TrainStart, "--end-date", $TrainEnd, "--negative-ratio", $NegativeRatio, "--sample-mode", $SampleMode)
...
$ValArgs = @("-m", "blackbox_finetune_recall80.build_validation_dataset", "--output-dir", $ValidationDir, "--start-date", $ValidationStart, "--end-date", $ValidationEnd, "--negative-ratio", $NegativeRatio, "--sample-mode", $SampleMode)
...
Invoke-Step -m blackbox_finetune_recall80.evaluate --base-model $BaseModel --adapter-dir "$OutputDir/adapter" --data-dir $ValidationDir --threshold 0.50 --min-positive-recall $MinRecall --cuda-device $CudaDevice --max-seq-length $MaxSeqLength
Invoke-Step -m unittest tests.test_blackbox_finetune_recall80 -v
