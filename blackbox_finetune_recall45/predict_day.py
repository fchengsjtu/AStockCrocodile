...
from blackbox_finetune_recall45.common import (
    DEFAULT_BASE_MODEL,
    DEFAULT_SAMPLE_MODE,
    DEFAULT_WINDOW,
    build_messages,
...
from blackbox_finetune_recall45.inference import load_model, score_prediction
...
def predict_day(
    ...
    with mysql_connect() as conn:
        ...
        print(f"blackbox recall45 predict date={anchor} symbols={len(symbols)} batches={len(batches)}", flush=True)
    ...
    if save_db:
        with mysql_connect() as conn:
            saved = save_top_predictions(conn, result, "blackbox_finetune_recall45", threshold, max_seq_length, save_top_n)
        print(json.dumps({"date": str(anchor), "saved_predictions": saved, "strategy": "blackbox_finetune_recall45"}, ensure_ascii=False), flush=True)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Predict one trading day with recall45 black-box model")
