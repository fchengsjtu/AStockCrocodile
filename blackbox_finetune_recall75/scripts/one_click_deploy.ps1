...
$VenvDir = if ($env:VENV_DIR) { $env:VENV_DIR } else { ".\.venv-blackbox-finetune-recall75" }
...
Invoke-Step -m pip install -r .\blackbox_finetune_recall75\requirements.txt
...
Invoke-Step -m blackbox_finetune_recall75.gpu --cuda-device $CudaDevice
Write-EnvironmentSnapshot
if ($Mode -eq "diagnose") {
  exit 0
}

$DataDir = if ($env:DATA_DIR) { $env:DATA_DIR } else { "blackbox_finetune_recall75/data_no_partial_week" }
$ValidationDir = if ($env:VALIDATION_DATA_DIR) { $env:VALIDATION_DATA_DIR } else { "blackbox_finetune_recall75/data_evaluation_no_partial_week" }
...
$DefaultOutputDir = if ($SampleMode -eq "short") { "blackbox_finetune_recall75/runs/qwen2.5-0.5b-blackbox-recall75-short-lora" } elseif ($SampleMode -eq "xlong") { "blackbox_finetune_recall75/runs/qwen2.5-0.5b-blackbox-recall75-xlong-lora" } elseif ($SampleMode -eq "xxlong") { "blackbox_finetune_recall75/runs/qwen2.5-0.5b-blackbox-recall75-xxlong-lora" } else { "blackbox_finetune_recall75/runs/qwen2.5-0.5b-blackbox-recall75-long-lora" }
...
$BuildArgs = @("-m", "blackbox_finetune_recall75.build_dataset", "--output-dir", $DataDir, "--start-date", $TrainStart, "--end-date", $TrainEnd, "--negative-ratio", $NegativeRatio, "--sample-mode", $SampleMode)
...
$ValArgs = @("-m", "blackbox_finetune_recall75.build_validation_dataset", "--output-dir", $ValidationDir, "--start-date", $ValidationStart, "--end-date", $ValidationEnd, "--negative-ratio", $NegativeRatio, "--sample-mode", $SampleMode)
...
$TrainArgs = @(
  '-m', 'blackbox_finetune_recall75.train', '--base-model', $BaseModel, '--data-dir', $DataDir, '--output-dir', $OutputDir,
