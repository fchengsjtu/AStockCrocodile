...
$DataDir = if ($env:DATA_DIR) { $env:DATA_DIR } else { "blackbox_finetune_recall75/data" }
$ValidationDir = if ($env:VALIDATION_DATA_DIR) { $env:VALIDATION_DATA_DIR } else { "blackbox_finetune_recall75/data_validation" }
$OutputDir = if ($env:OUTPUT_DIR) { $env:OUTPUT_DIR } else { "blackbox_finetune_recall75/runs/qwen2.5-0.5b-blackbox-recall75-lora" }
$MinRecall = if ($env:MIN_POSITIVE_RECALL) { $env:MIN_POSITIVE_RECALL } else { "0.75" }
$TrainSeed = if ($env:TRAIN_SEED) { $env:TRAIN_SEED } else { "20260575" }
