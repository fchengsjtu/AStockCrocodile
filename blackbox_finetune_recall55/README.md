...
The one-click scripts reuse existing `train.jsonl` and `test.jsonl` files in `blackbox_finetune_recall55/data_partial_week` and `blackbox_finetune_recall55/data_validation_partial_week`. After the first full dataset build, later full runs skip the expensive sample materialization step and go straight to training/evaluation.
...
Training also caches tokenized samples under `blackbox_finetune_recall55/data_partial_week/tokenized`. If `train.jsonl`, `BASE_MODEL`, and `MAX_SEQ_LENGTH` are unchanged, later training runs load the tokenized cache and skip the slow tokenization pass.
...
python -m blackbox_finetune_recall55.build_dataset `
  ...
  --end-date 20241231 `
  --negative-ratio 1.0 `
  --output-dir blackbox_finetune_recall55/data_partial_week `
  --daily-window 55 `
  --weekly-window 55 `
  --batch-size 80
```
...
python -m blackbox_finetune_recall55.build_validation_dataset `
  ...
  --end-date 20260430 `
  --negative-ratio 1.0 `
  --output-dir blackbox_finetune_recall55/data_validation_partial_week `
  --daily-window 55 `
  --weekly-window 55 `
  --batch-size 80
```
...
python -m blackbox_finetune_recall55.train \
  ...
  --data-dir blackbox_finetune_recall55/data_partial_week \
...
python -m blackbox_finetune_recall55.evaluate `
  ...
  --data-dir blackbox_finetune_recall55/data_validation_partial_week `
  --threshold 0.50 `
