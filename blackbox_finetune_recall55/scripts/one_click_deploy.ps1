...
$DataDir = if ($env:DATA_DIR) { $env:DATA_DIR } else { "blackbox_finetune_recall55/data" }
...
Invoke-Step -m blackbox_finetune_recall55.train --base-model $BaseModel --data-dir $DataDir --output-dir $OutputDir --max-seq-length $MaxSeqLength --epochs $Epochs --batch-size 1 --gradient-accumulation-steps $GradSteps --learning-rate $LearningRate --max-grad-norm $MaxGradNorm --checkpoint-every $CheckpointEvery --train-seed $TrainSeed --cuda-device $CudaDevice --no-4bit
