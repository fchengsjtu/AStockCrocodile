...
if [[ "$RESET_THREECLASS_ENV" =~ ^(1|true|TRUE|yes|YES|y|Y)$ ]]; then
  for name in \
    ...
    RANK_LOSS_WEIGHT RANK_MARGIN NEUTRAL_RANK_LOSS_WEIGHT NEUTRAL_RANK_MARGIN POSITIVE_HIGH_SCORE_LOSS_WEIGHT POSITIVE_HIGH_SCORE_MARGIN \
    HIGH_SCORE_EMA HIGH_SCORE_EMA_ALPHA \
    HIGH_SCORE_CUTOFF_POSITION HIGH_SCORE_POSITIVE_BONUS HIGH_SCORE_POSITIVE_BONUS_SCALE HIGH_SCORE_POSITIVE_BONUS_MAX_MULTIPLIER \
...
TRAIN_ARGS=(
  ...
  --neutral-rank-loss-weight "$NEUTRAL_RANK_LOSS_WEIGHT"
  --neutral-rank-margin "$NEUTRAL_RANK_MARGIN"
  --positive-high-score-loss-weight "$POSITIVE_HIGH_SCORE_LOSS_WEIGHT"
  --positive-high-score-margin "$POSITIVE_HIGH_SCORE_MARGIN"
  --high-score-ema-alpha "$HIGH_SCORE_EMA_ALPHA"
  --high-score-cutoff-position "$HIGH_SCORE_CUTOFF_POSITION"
  --high-score-positive-bonus "$HIGH_SCORE_POSITIVE_BONUS"
  --high-score-positive-bonus-scale "$HIGH_SCORE_POSITIVE_BONUS_SCALE"
  ...
  --fp-threshold-ema-alpha "$FP_THRESHOLD_EMA_ALPHA"
...
if env_flag "$HIGH_SCORE_EMA"; then
  TRAIN_ARGS+=(--high-score-ema)
else
  TRAIN_ARGS+=(--no-high-score-ema)
fi
