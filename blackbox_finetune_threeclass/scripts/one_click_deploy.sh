...
if [[ "$RESET_THREECLASS_ENV" =~ ^(1|true|TRUE|yes|YES|y|Y)$ ]]; then
  for name in \
    ...
    POSITIVE_PURIFICATION_ENABLED POSITIVE_PURIFICATION_GROUP_SIZE POSITIVE_PURIFICATION_BOTTOM_K POSITIVE_PURIFICATION_DECAY \
    SELECTED_GROUPS_ENABLED SELECTED_GROUPS_OUTPUT_DIR \
    CHECKPOINT_EVERY NO_AUTO_RESUME TRAIN_SEED EVAL_RANDOM_SEED \
...
for name in \
  ...
  HIGH_SCORE_NEGATIVE_PENALTY_WEIGHT HIGH_SCORE_NEUTRAL_PENALTY_WEIGHT \
  HIGH_SCORE_NEGATIVE_MARGIN HIGH_SCORE_NEUTRAL_MARGIN \
  POSITIVE_PURIFICATION_ENABLED POSITIVE_PURIFICATION_GROUP_SIZE \
  POSITIVE_PURIFICATION_BOTTOM_K POSITIVE_PURIFICATION_DECAY \
  SELECTED_GROUPS_ENABLED SELECTED_GROUPS_OUTPUT_DIR \
  CHECKPOINT_EVERY NO_AUTO_RESUME TRAIN_SEED EVAL_RANDOM_SEED \
  EVAL_SAMPLE_METHOD EVAL_MAX_SAMPLES EVAL_PRECISION_TOP_K \
  EVAL_PRECISION_THRESHOLD NEGATIVE_WEIGHT NEUTRAL_WEIGHT EVAL_THRESHOLD_TOP_RATIO \
...
if env_flag "$POSITIVE_PURIFICATION_ENABLED"; then
  TRAIN_ARGS+=(--positive-purification-enabled)
else
  TRAIN_ARGS+=(--no-positive-purification-enabled)
fi
if env_flag "$SELECTED_GROUPS_ENABLED"; then
  TRAIN_ARGS+=(--selected-groups-enabled)
fi
if [[ -n "$SELECTED_GROUPS_OUTPUT_DIR" ]]; then
  TRAIN_ARGS+=(--selected-groups-output-dir "$SELECTED_GROUPS_OUTPUT_DIR")
fi
