...
from blackbox_finetune.evaluate import load_model, score_prediction
...
def predict_day(
    ...
    with mysql_connect() as conn:
        ...
        print(f"blackbox predict date={anchor} symbols={len(symbols)} batches={len(batches)}", flush=True)
        for batch_index, batch in enumerate(batches, start=1):
            ...
            for scode in batch:
                ...
                if pred["label"] != "positive" or pred["positive_probability"] < threshold:
                    continue
                rows.append(
                    {
                        "TradeDate": anchor,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Predict all stocks for one date with black-box fine-tuned model")
