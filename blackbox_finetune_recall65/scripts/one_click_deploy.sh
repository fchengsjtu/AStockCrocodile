...
python -m blackbox_finetune_recall65.build_dataset "${BUILD_ARGS[@]}"
python -m blackbox_finetune_recall65.build_validation_dataset "${VAL_ARGS[@]}"
python -m blackbox_finetune_recall65.train --base-model "$BASE_MODEL" --data-dir "$DATA_DIR" --output-dir "$OUTPUT_DIR" --max-seq-length "$MAX_SEQ_LENGTH" --epochs "$EPOCHS" --batch-size 1 --gradient-accumulation-steps "$GRAD_STEPS" --learning-rate 2e-4 --train-seed "$TRAIN_SEED" --cuda-device "$CUDA_DEVICE"
