...
def predict_day(
    ...
    with mysql_connect() as conn:
        ...
        print(f"blackbox recall60 predict date={anchor} symbols={len(symbols)} batches={len(batches)}", flush=True)
    ...
    if save_db:
        with mysql_connect() as conn:
            saved = save_top_predictions(conn, result, "blackbox_finetune_recall60", threshold, max_seq_length, save_top_n)
        print(json.dumps({"date": str(anchor), "saved_predictions": saved, "strategy": "blackbox_finetune_recall60"}, ensure_ascii=False), flush=True)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Predict one trading day with recall60 black-box model")
