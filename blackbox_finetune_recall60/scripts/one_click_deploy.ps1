...
$BuildArgs = @("-m", "blackbox_finetune_recall60.build_dataset", "--output-dir", $DataDir, "--start-date", $TrainStart, "--end-date", $TrainEnd, "--negative-ratio", "1.0")
if ($PositiveLimit) { $BuildArgs += @("--positive-limit", $PositiveLimit) }
Invoke-Step @BuildArgs

$ValArgs = @("-m", "blackbox_finetune_recall60.build_validation_dataset", "--output-dir", $ValidationDir, "--start-date", $ValidationStart, "--end-date", $ValidationEnd, "--negative-ratio", "1.0")
