...
$DataDir = if ($env:DATA_DIR) { $env:DATA_DIR } else { "blackbox_finetune_recall35/data" }
$ValidationDir = if ($env:VALIDATION_DATA_DIR) { $env:VALIDATION_DATA_DIR } else { "blackbox_finetune_recall35/data_validation" }
$OutputDir = if ($env:OUTPUT_DIR) { $env:OUTPUT_DIR } else { "blackbox_finetune_recall35/runs/qwen2.5-0.5b-blackbox-recall35-lora" }
$MinRecall = if ($env:MIN_POSITIVE_RECALL) { $env:MIN_POSITIVE_RECALL } else { "0.35" }
$TrainSeed = if ($env:TRAIN_SEED) { $env:TRAIN_SEED } else { "20260535" }
...
$BuildArgs = @("-m", "blackbox_finetune_recall35.build_dataset", "--output-dir", $DataDir, "--start-date", $TrainStart, "--end-date", $TrainEnd, "--negative-ratio", "1.0")
if ($PositiveLimit) { $BuildArgs += @("--positive-limit", $PositiveLimit) }
Invoke-Step @BuildArgs

$ValArgs = @("-m", "blackbox_finetune_recall35.build_validation_dataset", "--output-dir", $ValidationDir, "--start-date", $ValidationStart, "--end-date", $ValidationEnd, "--negative-ratio", "1.0")
if ($Mode -eq "smoke") { $ValArgs += @("--positive-limit", $PositiveLimit) }
Invoke-Step @ValArgs

Invoke-Step -m blackbox_finetune_recall35.train --base-model $BaseModel --data-dir $DataDir --output-dir $OutputDir --max-seq-length $MaxSeqLength --epochs $Epochs --batch-size 1 --gradient-accumulation-steps $GradSteps --learning-rate $LearningRate --max-grad-norm $MaxGradNorm --checkpoint-every $CheckpointEvery --train-seed $TrainSeed --cuda-device $CudaDevice --no-4bit
