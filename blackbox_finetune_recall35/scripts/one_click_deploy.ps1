...
$DataDir = if ($env:DATA_DIR) { $env:DATA_DIR } else { "blackbox_finetune_recall35/data_no_partial_week" }
$ValidationDir = if ($env:VALIDATION_DATA_DIR) { $env:VALIDATION_DATA_DIR } else { "blackbox_finetune_recall35/data_evaluation_no_partial_week" }
...
$BuildArgs = @("-m", "blackbox_finetune_recall35.build_dataset", "--output-dir", $DataDir, "--start-date", $TrainStart, "--end-date", $TrainEnd, "--negative-ratio", "3.0", "--sample-mode", $SampleMode)
...
$ValArgs = @("-m", "blackbox_finetune_recall35.build_validation_dataset", "--output-dir", $ValidationDir, "--start-date", $ValidationStart, "--end-date", $ValidationEnd, "--negative-ratio", "3.0", "--sample-mode", $SampleMode)
