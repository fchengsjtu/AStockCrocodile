...
from blackbox_finetune_recall70.common import DEFAULT_BASE_MODEL, DEFAULT_DATA_DIR, DEFAULT_OUTPUT_DIR, DEFAULT_TRAIN_SEED, compact_messages_from_sample
...
def _missing_dataset_error(data_dir: Path) -> FileNotFoundError:
    return FileNotFoundError(f"missing dataset files under {data_dir}; run blackbox_finetune_recall70.build_dataset first")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--no-4bit", action="store_true", help="Accepted for script compatibility; recall70 Windows training uses fp16 LoRA on RTX3060.")
