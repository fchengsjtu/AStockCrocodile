...
$VenvDir = if ($env:VENV_DIR) { $env:VENV_DIR } else { ".\.venv-blackbox-finetune-recall70" }
...
Invoke-Step -m pip install -r .\blackbox_finetune_recall70\requirements.txt
...
Invoke-Step -m blackbox_finetune_recall70.gpu --cuda-device $CudaDevice
...
$DefaultDataDir = "blackbox_finetune_recall70/data_no_partial_week_$SampleMode"
$DefaultValidationDir = "blackbox_finetune_recall70/data_evaluation_no_partial_week_$SampleMode"
...
$DefaultOutputDir = if ($SampleMode -eq "short") { "blackbox_finetune_recall70/runs/qwen2.5-0.5b-blackbox-recall70-short-lora" } elseif ($SampleMode -eq "xlong") { "blackbox_finetune_recall70/runs/qwen2.5-0.5b-blackbox-recall70-xlong-lora" } elseif ($SampleMode -eq "xxlong") { "blackbox_finetune_recall70/runs/qwen2.5-0.5b-blackbox-recall70-xxlong-lora" } else { "blackbox_finetune_recall70/runs/qwen2.5-0.5b-blackbox-recall70-long-lora" }
...
$BuildArgs = @("-m", "blackbox_finetune_recall70.build_dataset", "--output-dir", $DataDir, "--start-date", $TrainStart, "--end-date", $TrainEnd, "--negative-ratio", $NegativeRatio, "--sample-mode", $SampleMode)
...
$ValArgs = @("-m", "blackbox_finetune_recall70.build_validation_dataset", "--output-dir", $ValidationDir, "--start-date", $ValidationStart, "--end-date", $ValidationEnd, "--negative-ratio", $NegativeRatio, "--sample-mode", $SampleMode)
...
$TrainArgs = @(
  '-m', 'blackbox_finetune_recall70.train', '--base-model', $BaseModel, '--data-dir', $DataDir, '--output-dir', $OutputDir,
...
Invoke-Step -m blackbox_finetune_recall70.evaluate --base-model $BaseModel --adapter-dir "$OutputDir/adapter" --data-dir $ValidationDir --threshold 0.50 --min-positive-recall $MinRecall --cuda-device $CudaDevice --max-seq-length $MaxSeqLength
Invoke-Step -m unittest tests.test_blackbox_finetune_recall70 -v
