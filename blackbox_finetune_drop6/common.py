...
from blackbox_finetune.common import DEFAULT_BASE_MODEL, DEFAULT_WINDOW, SampleEvent
from llm_finetune.common import compact_date, iter_batches, load_kline_map, parse_date

PROJECT_DIR = Path("blackbox_finetune_drop6")
DEFAULT_STAT_TYPE = "short_term_drop_3d_6pct"
DEFAULT_MODEL_TAG = "drop6"
...
DEFAULT_PRECISION_TOP_K = 10
DEFAULT_PRECISION_THRESHOLD = 0.40
REPORTED_PRECISION_KS = (5, 10, 20, 50, 100, 200, 500)
DEFAULT_TRAIN_SEED = 937498347
CSV_COLUMNS = "dt/o/h/l/c/v/a/m5/m13/m34/m55"
SYSTEM_PROMPT = "Classify A-share 3-day drawdown risk. Return JSON."
SHORT_SAMPLE_MODE = "short"
LONG_SAMPLE_MODE = "long"
XLONG_SAMPLE_MODE = "xlong"
XXLONG_SAMPLE_MODE = "xxlong"
DEFAULT_SAMPLE_MODE = XLONG_SAMPLE_MODE
SAMPLE_MODES = {
...
def model_tag(value: str | None = None) -> str:
    return (value or os.environ.get("MODEL_TAG") or DEFAULT_MODEL_TAG).strip() or DEFAULT_MODEL_TAG


def normalize_recall_target(value: str | float | int | None = None) -> float:
    # Compatibility shim for copied recall60 prediction scripts. The drop6
    # package uses a fixed model tag, so this value is informational only.
    del value
    return 0.06
...
def recall_target_tag(value: str | float | int | None = None) -> str:
    # Kept for compatibility with copied recall60 training/evaluation code.
    return model_tag(str(value) if value is not None else None)
...
def default_data_dir(sample_mode: str | None = None) -> Path:
    ...
    return PROJECT_DIR / f"data_no_partial_week_{model_tag()}_{mode}"


def default_validation_dir(sample_mode: str | None = None) -> Path:
    mode = normalize_sample_mode(sample_mode or os.environ.get("SAMPLE_MODE"))
    return PROJECT_DIR / f"data_evaluation_no_partial_week_{model_tag()}_{mode}"
...
def default_output_dir(sample_mode: str | None = None) -> Path:
    mode = normalize_sample_mode(sample_mode or os.environ.get("SAMPLE_MODE"))
    return PROJECT_DIR / "runs" / f"qwen2.5-0.5b-blackbox-{model_tag()}-{mode}-lora"
