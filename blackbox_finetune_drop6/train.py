...
from blackbox_finetune_drop6.inference import score_prediction
...
def _missing_dataset_error(data_dir: Path) -> FileNotFoundError:
    return FileNotFoundError(f"missing dataset files under {data_dir}; run blackbox_finetune_drop6.build_dataset first")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fine-tune Qwen2.5 for drop6 black-box stock classification")
    ...
    parser.add_argument("--no-4bit", action="store_true", help="Accepted for script compatibility; drop6 Windows training uses fp16 LoRA on RTX3060.")
