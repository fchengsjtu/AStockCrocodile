...
VENV_DIR="${VENV_DIR:-$HOME/.venvs/astock-blackbox-finetune-drop6}"
...
python -m pip install -r blackbox_finetune_drop6/requirements.txt
...
python -m blackbox_finetune_drop6.gpu --cuda-device "$CUDA_DEVICE"
if [[ "$MODE" == "diagnose" ]]; then
  exit 0
fi
MODEL_LABEL_VALUE="$(model_label_value "${MODEL_LABEL_VALUE:-drop6}")"
...
DEFAULT_DATA_DIR="blackbox_finetune_drop6/data_no_partial_week_${MODEL_TAG}_${SAMPLE_MODE}_${NEGATIVE_TAG}"
DEFAULT_VALIDATION_DATA_DIR="blackbox_finetune_drop6/data_evaluation_no_partial_week_${MODEL_TAG}_${SAMPLE_MODE}_${NEGATIVE_TAG}"
DATA_DIR="${DATA_DIR:-$DEFAULT_DATA_DIR}"
VALIDATION_DATA_DIR="${VALIDATION_DATA_DIR:-$DEFAULT_VALIDATION_DATA_DIR}"
DEFAULT_OUTPUT_DIR="blackbox_finetune_drop6/runs/qwen2.5-0.5b-blackbox-${MODEL_TAG}-${SAMPLE_MODE}-lora"
...
Project blackbox environment:
  ...
  NEGATIVE_RULE=exclude samples within +/-10 trading days of any positive for same stock
...
if [[ "$EXPLICIT_DATASETS_ENABLED" == "1" ]]; then
  ...
  run_dataset_build_if_needed "$DATA_DIR" training "${REBUILD_DATASET:-}" "$TRAIN_DATASET_SIGNATURE" python -m blackbox_finetune_drop6.build_dataset "${BUILD_ARGS[@]}"
  run_dataset_build_if_needed "$VALIDATION_DATA_DIR" validation "${REBUILD_VALIDATION_DATASET:-${REBUILD_DATASET:-}}" "$VALIDATION_DATASET_SIGNATURE" python -m blackbox_finetune_drop6.build_validation_dataset "${VAL_ARGS[@]}"
fi
...
TRAIN_HELP="$(python -m blackbox_finetune_drop6.train --help 2>&1 || true)"
