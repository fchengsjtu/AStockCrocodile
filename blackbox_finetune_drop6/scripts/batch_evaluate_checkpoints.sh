...
EVAL_DATA_DIR="${2:-blackbox_finetune_drop6/data_evaluation_no_partial_week_drop6_xlong_neg9}"
OUTPUT_ROOT="${3:-}"
...
if [[ -z "${PYTHON_BIN:-}" ]]; then
  ...
  elif [[ -x "/home/fcheng/.venvs/astock-blackbox-finetune-drop6/bin/python" ]]; then
    PYTHON_BIN="/home/fcheng/.venvs/astock-blackbox-finetune-drop6/bin/python"
  else
    PYTHON_BIN="python3"
  fi
fi
...
echo "Batch evaluating drop6 checkpoints"
...
PY
  ...
  set +e
  "$PYTHON_BIN" -m blackbox_finetune_drop6.evaluate \
